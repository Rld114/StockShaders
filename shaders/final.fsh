...
vec3       REINHARD(vec3 color)                                                    { float luma = Luminance(color); float lumaSqrd = luma * luma; return color * ((luma + lumaSqrd * 0.25) / (luma + lumaSqrd)); }
...
void Vignette(inout vec3 color) {
    ...
    color *= vig;
	//float dist = distance(texcoord.st, vec2(0.5f)) * 5.0f;
	//	  dist /= 4.6142f;
...
void 	AddRainFogScatter(inout vec3 color, in BloomDataStruct bloomData)
{
  ...
  const float howdense = 8;
#elif RAIN_ATMOSPHERE == 1
  const float howdense = 1;
#endif

	float fogDensity = 0.007f * (rainStrength * howdense);
...
void EyeBrightness(inout vec3 color) {
  float avglod;
	avglod = int(log2(min(viewWidth, viewHeight))) - 0;

	float avgLumPow = 1.1;
	float exposureMax = 1.0;
	float exposureMin = 0.00055;
...
vec2 temperaturecolor = mix(vec2(COLOR_TEMP), vec2(35000), vec2(mix(0.0f, 1.0f, timeMidnight)));
//White balancing
 color *= colorTemperatureToRGB(temperaturecolor.x);
//Contrast
