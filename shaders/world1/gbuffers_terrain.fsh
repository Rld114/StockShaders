...
void main() {	
	...
	if (abs(materialIDs - 20.0f) < 0.1f || abs(materialIDs - 21.0f) < 0.1f || abs(materialIDs - 35.0f) < 0.1f)
	{
		spec.g = 0.0f;
	}
	else
	...
	if (distance < bump_distance) {
			...
			//bump += CalculateRainBump(worldPosition.xyz  + cameraPosition.xyz);
