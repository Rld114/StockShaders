...
vec3 	CalculateSunlightVisibility(inout SurfaceStruct surface, in ShadingStruct shadingStruct) {				//Calculates shadows
	...
	if (shadingStruct.direct > 0.0f) {
		...
		#ifdef RT_SHADOWS
		const float Shadow_res = shadowMapResolution * 125;
		float res_tes = 2048 * 125;
		#else
		...
		if (shadowMult > 0.0) 
		{
			...
			// 0 = off 1 = vps 2 = soft shadow
				...
				vec3 noise = toRandPerFrame(rand(texcoord.st), frameTimeCounter) * CalculateNoisePattern1(rand(vec2(0.0f)).xy, 64);

				float angle = (CalculateDitherPattern1() * 2) * 3.14159 * 2.0;
				...
				int count = 0;
				float spread = penumbraSize * 0.0125 * vpsSpread + 0.5 / Shadow_res;
		...
		vec3 noise3 = toRandPerFrame(rand(texcoord.st), frameTimeCounter) * CalculateNoisePattern1(rand(vec2(0.0f)).xy, 64);
		...
		#endif
		#ifdef RT_SHADOWS
			result = mix(vec3(1.0), result, (shadowMult + (edgenoises.x - 0.1)));
		#else
...
void 	CalculateRainFog(inout vec3 color, in SurfaceStruct surface)
{
	...
	float fogDensity = 0.00998f * rainStrength;
...
{
	vec3 fogColor = pow(colorSkylight * (1.0 - timeMidnight), vec3(0.55f));
...
vec4 CloudColor2(in vec4 worldPosition, in float sunglow, in vec3 worldLightVector, in float altitude, in float heightFactor, const bool isShadowPass)
{
	...
	float coverage = CL_COVERAGE;
		  ...
		  coverage *= max(0.0f, 1.0f - dist / 4000.0f);
	float density = CL_DENSITY - rainStrength * 0.3;
...
void CloudPlane(SurfaceStruct surface, inout vec3 color)
{
	...
	float sunglow = CalculateSunglow(surface) + CL_SCATTERING;
...
void CrepuscularRays(inout float color, in SurfaceStruct surface)
{
	...
	const float rayLimit = 35;// 280.0f;
	...
	color = lightAccumulation * 0.2 * mix(0.0, 1.0, eyeBrightnessSmooth.y / 240.0f);
...
vec3 toneSaturation(vec3 col, float a){
	...
	return Luminance(col) * (1.0 - a) + col * a;
}
void main() {
	...
	surface.sky.albedo 		= vec3(0.00005);							//Gets the albedo texture for the sky
	...
	//		shading.sunlightVisibility 	= CalculateSunlightVisibility(surface, shading) * contactShadows(viewPos.xyz, surface.normal, surface.mask);
	//	#else
	//		shading.sunlightVisibility 	=CalculateSunlightVisibility(surface, shading);
	//	#endif
	//#else
	//	#ifdef CONTACT_SHADOWS
	//		shading.sunlightVisibility 	= vec3(1) * contactShadows(viewPos.xyz, surface.normal, surface.mask);
	//	#else
	...
	vec3 finalComposite = final.sunlight 			* 0.7f 	* 5.5f				//Add direct sunlight
						+ final.skylight 			* 5.04f				//Add ambient skylight
						+ final.nolight 			* 0.00401f 			//Add base ambient light
						+ final.scatteredSunlight 	* 5.02f		* (1.0f - sunlightMult)					//Add fake scattered sunlight
	...
	//if (rainStrength > 0.1 && isEyeInWater == 0) {
	//	CalculateRainFog(finalComposite.rgb, surface);
	//}

	//surface.sky.albedo *= 16.0f;
	...
	finalComposite *= 0.0001f;												//Scale image down for HDR
	finalComposite.b *= 4.2f;
