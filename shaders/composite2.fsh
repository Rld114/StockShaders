...
void 	CalculateMasks(inout MaskStruct mask) {
	...
	mask.redstoneblock  = GetMaterialMask(texcoord.st, 35, mask.matIDs);
...
vec4 	ComputeRaytraceReflection(inout SurfaceStruct surface)
{
	...
	vec3 dither = toRandPerFrame(rand(texcoord.st), FRAME_TIME) * CalculateNoisePattern1(rand(vec2(0.0f)).xy, 4);
    ...
    vec3 cameraSpaceNormal = surface.normal;
    	 cameraSpaceNormal += mix(dither * TRACENOISE * surface.roughness, vec3(0.0), rainStrength);
	...
	#ifdef HQ_SSR
		const int maxRefinements = 64;
		const int Steps = 2048;
	#else
		const int maxRefinements = 6;
		const int Steps = 65;
	#endif
	int numSteps = 0;////////////////////////////          reduce step
	...
	color = pow(texture2DLod(gcolor, finalSamplePos, 2), vec4(2.2f));
...
vec3 	ComputeReflectedSkyGradient(in SurfaceStruct surface) {
	...
	skyColor = mix(skyColor, colorSunlight * 2.0, pow(sunglow, 6.0) * (1.05 - rainStrength));
...
void 	CalculateSpecularReflections(inout SurfaceStruct surface) {
	...
	if (surface.mask.diamondBlock)
	{
		//surface.specularity = 1.0f;
		surface.roughness = 0.4f;
		surface.baseSpecularity = 1.0f;
		surface.specularColor = vec3(0.2f, 0.5f, 0.9f);
		surface.specularColor = mix(surface.specularColor, vec3(1.0f), vec3(0.015f));
	}

	if (surface.mask.emeraldBlock)
	{
		//surface.specularity = 1.0f;
		surface.roughness = 0.4f;
		surface.baseSpecularity = 1.0f;
		surface.specularColor = vec3(0.1f, 1.0f, 0.002f);
		surface.specularColor = mix(surface.specularColor, vec3(1.0f), vec3(0.015f));
	}

	if (surface.mask.redstoneblock)
	{
		//surface.specularity = 1.0f;
		surface.roughness = 0.4f;
		surface.baseSpecularity = 1.0f;
		surface.specularColor = vec3(1.0f, 0.0f, 0.002f);
		surface.specularColor = mix(surface.specularColor, vec3(1.0f), vec3(0.015f));
	}
...
void main() {
	...
	vec4 viewPos 					= GetViewPositionRaw(texcoord.st, surface.depth);
	vec4 worldPos					= gbufferModelViewInverse * vec4(viewPos.xyz, 0.0);
	vec3 worldDir 					= normalize(worldPos.xyz);

	#if SURFACE_REFLECTANCE == 0
		surface.specularity = GetSpecularity(texcoord.st);
	#elif SURFACE_REFLECTANCE == 1
		surface.specularity = 0.55 + (GetSpecularity(texcoord.st) * 0.15);
	#endif

	#if SURFACE_ROUGH == 2
		surface.roughness = 0.52 - GetRoughness(texcoord.st);
	#elif SURFACE_ROUGH == 1
		surface.roughness = 0.82 - GetRoughness(texcoord.st);
	#elif SURFACE_ROUGH == 0
		surface.roughness = 1.0 - GetRoughness(texcoord.st);
	#endif
	...
	if (surface.mask.ironBlock)
	{
		surface.specularity = 0.9f;
		surface.roughness = 0.0f;
	}

	if (surface.mask.goldBlock || surface.mask.diamondBlock || surface.mask.emeraldBlock)
	{
		surface.specularity = 0.98f;
	...
	#ifdef SMOOTH_CLOUDS
		surface.cloudAlpha = texture2D(composite, texcoord.st, 1).g;
		SmoothSky(surface);
	#endif

	if (surface.mask.water || surface.mask.ice)
		surface.sunlightVisibility = vec3(2.0);
	...
	{
		surface.color *= transparentAlbedo.rgb + 0.02;
		surface.roughness = TRANSPARENT_ROUGHNESS;
		surface.specularity = TRANSPARENT_REFLECTANCE;
	}
	...
	gl_FragData[0] = vec4(surface.color, totalInternalReflection);
